...
class RealtimeDataManager {
  ...
  startPriceUpdates(connectionId: string, options: RealtimeDataOptions): void {
    console.log(`DEPRECATED: startPriceUpdates called for ${connectionId} - use simple-realtime.ts instead`);
  ...
  private async fetchPrices(connectionId: string): Promise<void> {
    console.log(`DEPRECATED: fetchPrices called for ${connectionId} - no API calls made`);
   ...
   * DEPRECATED: No longer makes API calls
   */
  async getEnhancedRealtimeData(coinIds: string[], currency: string): Promise<CoinData[]> {
    console.log(`DEPRECATED: getEnhancedRealtimeData called - no API calls made`);
...
export function useRealtimePrices(coinIds: string[], currency: string, updateInterval: number = 30000) {
