...
export function useRealBinanceWebSocket(page: number = 0) {
  ...
  const setupWebSocketHandlers = useCallback((ws: WebSocket, connectionName: string) => {
    ws.onopen = () => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.stream && data.data) {
          ...
          console.log(
            `${connectionName} - Received update for ${tickerData.s}: $${tickerData.c}`
          );
          updatePrice(tickerData);
        } else if (data.s) {
          // Single stream format (fallback)
          console.log(
            `${connectionName} - Received direct update for ${data.s}: $${data.c}`
          );
          updatePrice(data);
        } else {
        ...
        console.error(
          `${connectionName} - Error parsing WebSocket message:`,
          err,
          event.data
        );
      }
    ...
    ws.onclose = (event) => {
      console.log(
        `${connectionName} - WebSocket closed: ${event.code} - ${event.reason}`
      );
      setConnectionsActive((prev) => {
      ...
      if (event.code !== 1000 && reconnectAttempts.current < maxReconnectAttempts) {
        ...
        reconnectTimeoutRef.current = setTimeout(() => {
          ...
          // Just set error state, the useEffect will handle reconnection
